...
@bot.on(admin_cmd(pattern="sh(?: |$|\n)(.*)", command="exec"))
@bot.on(sudo_cmd(pattern="sh(?: |$|\n)(.*)", command="exec", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern="py?: |$|\n)(.*)", command="eval"))
@bot.on(sudo_cmd(pattern="py(?: |$|\n)(.*)", command="eval", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cmd = "".join(event.text.split(maxsplit=1)[1:])
    if not cmd:
        return await edit_delete(event, "`What should i run ?..mtlb kya run krna h?..`")
